# Infinite Studio（万能 AI 工坊）设计文档

- 日期：2026-06-03（重整：聚焦第一期 MVP，表设计定稿）
- 状态：设计草案。**本方案正文只覆盖第一期 MVP（12 张表）**；计费、负载均衡、应用中心、OAuth/RBAC、规模化等**所有未来演进**集中在末章 §10「未来演进（分块导论）」，仅留方向与要点 hint。
- 作者：lixinqiany（与 Claude 头脑风暴整理）
...
**目标定位（按优先级）：**
1. **最终是一款产品**：可分发给亲友体验，做得好可商业化（计费模型「API 用量 → 平台统一积分」，**计费本期不做**，见 §10.1）。
...
- **代码适配层**（`ProviderAdapter` + 各 vendor 实现，**内部用各家官方 SDK**）= 逻辑：怎么调、怎么归一化用量。`provider.vendor` 字段把「数据」选到「代码适配器」。
- 密钥只在「你的后端 ↔ 该密钥本属的官方 vendor」之间流动，**不经任何第三方聚合层**；借 LiteLLM 的 model_list / Dify 的加密存储**设计思路**，但调用自己用官方 SDK 写。

> MinIO、Celery+Redis、pgvector 等本期不引入（§10.7）。
...
┌──────────┐   HTTP / SSE(流式)   ┌─────────────────────────────────────┐
│ Vue3 前端 │ ───────────────────▶ │             FastAPI 后端              │
│ 对话视图  │ ◀─────────────────── │ 鉴权 · agent(react_loop) · 用量记录    │
│ 管理后台  │                      │ provider 适配层(官方SDK) ──┐          │
└──────────┘                      └──────┬──────────────────┬─┴──────────┘
                                         ▼                  ▼
                                    ┌────────┐     ┌──────────────┐
                                    │Postgres│     │ 模型供应商API  │
                                    ...
                                    └────────┘     └──────────────┘
```
- **单服务起步**；对话 SSE 直出，无队列、无文件、不计费。
- chat = `react_loop`：LLM 决策 → 需要时调工具（tavily）→ 结果回灌 → 继续，受 `max_turns` 约束。
- 每次模型/工具调用精确记 `usage_event`（不扣费）。

---

## 4. 数据层纪律

> **Postgres 是唯一事实源；每加专用存储须有「访问模式 + 规模」理由；派生数据要有同步路径；图片/文件只存元数据+URL，绝不存 base64。** 向量、海量分析、全文检索、队列等按规模触发点逐个加（§10.7）。
...
- **主键**：
  - **用户数据实体**（users 等，自然键如 username 会变）→ 代理键 `id bigint` + 自然键加 `UNIQUE`。
  - **代码引用的注册表**（agent / tool / provider / model，`key` 不可变、处处按 key 引用）→ **`key` 做 PK**（零 id↔key 间接层）。
  - 后期可拓展：对外暴露不可枚举 id 加 `public_id uuid`；分库时把 id 生成换分布式 ID（Snowflake/Leaf），表结构不变。

| # | 域 | 表 | 用途 |
|---|---|---|---|
| 1 | 用户 | `users` | 账户（用户名/角色/状态/邀请码来源） |
| 2 | 用户 | `auth_identity` | 登录方式（密码 / 未来 OAuth） |
| 3 | 用户 | `auth_session` | 服务端会话（按设备一行） |
| 4 | 用户 | `system_code` | 管理员维护的注册/促销码 |
| 5 | 能力 | `agent` | agent 定义（声明式配置；逻辑在代码） |
| 6 | 能力 | `tool` | 工具注册表（code-first 目录） |
| 7 | 模型 | `provider` | 供应商连接（厂牌 + 加密密钥） |
| 8 | 模型 | `model` | 逻辑模型（挂到一个供应商） |
| 9 | 对话 | `conversation` | 会话 |
| 10 | 对话 | `run` | 一次执行 |
| 11 | 对话 | `item` | run 内有序事件（只追加） |
| 12 | 用量 | `usage_event` | 模型 + 工具用量快照（**记录、不扣费**） |
...
**① `users` —— 用户账户**（避开 Postgres 保留字 `user`）
```
id                    bigint        PK
...
- 密码不在此表（在 `auth_identity`）。角色判断**收口在统一函数**（`require_permission(...)`），不散写 `if role=='admin'`，将来无损迁 RBAC（§10.6）。
...
**④ `system_code` —— 系统签发的码**（管理员维护：邀请码 now / 促销码 later）
```
id                  bigint        PK
...
**⑤ `agent` —— agent 定义**（= DB 版的 AgentDefinition；`key` 做 PK）
```
key             varchar  PK                  稳定引用键（代码按 key resolve，如 'chat'）
name            varchar  NOT NULL
description     varchar  NULL                 （未来 handoff 路由"何时该用我"）
runtime_pattern varchar  NOT NULL  CHECK IN ('react_loop')   代码已注册逻辑；MVP 仅单 agent+工具循环（纯对话=空工具）
model           varchar  NULL                 引用 model.key；**无代码默认**，解析时为空/失效则报错（fail-fast）
instructions    text     NULL                 系统提示词；**NULL = 继承代码默认**（overlay）
model_params    jsonb    NOT NULL default '{}'  不规则采样参数(temperature/top_p/...)，缺则用供应商自身默认
allowed_tool_keys text[] NOT NULL default '{}' 可调工具 key（引用 tool.key，如 ['tavily_search']）
...
- `model_params` 缺 → 用供应商自身默认（非我们硬编码）。

**⑥ `tool` —— 工具注册表**（code-first 目录；`key` 做 PK）
```
key           varchar  PK                  代码写死的不可变标识；agent.allowed_tool_keys / item.tool_key / LLM 都按它引用
name          varchar  NOT NULL
...
updated_at    timestamptz NOT NULL default now()
```
MVP seed 一个 `tavily_search`（搜索工具，逻辑后续开发）。

**写入/留存生命周期（两表机制不同，黄金法则：启动绝不覆盖「人配的那一层」）：**
- **`tool`（代码拥有定义、admin 只调策略）**：启动按 key **upsert**，但 `SET` 只写**代码字段**（name/description/schema），**不碰**管理员字段（enabled/未来定价）→ 管理员策略跨重启不丢。代码删工具 → 标 deprecated/禁用不硬删；代码加工具 → 下次启动出现。
- **`agent`（代码兜底 + DB 覆盖）**：启动**不写** agent 表。`resolve_agent(key)` = **DB 行 ?? 代码默认(提示词/逻辑) ?? 全局 fallback**。`model` 必须由配置提供（无代码默认 → 缺则报错）。**启动从不覆盖 admin 的行 → 配过的提示词跨部署存活**。

### 5.4 模型层：`provider` / `model`

**⑦ `provider` —— 供应商连接**（admin 配，不 seed，密钥加密；`key` 做 PK）
```
key            varchar    PK              连接标识（如 'openai-main'）
vendor         varchar    NOT NULL        CHECK IN ('openai','anthropic','google')；选代码适配器
name           varchar    NOT NULL
...
**⑧ `model` —— 逻辑模型**（挂到一个供应商；`key` 做 PK；agent 引用它）
```
key              varchar  PK              agent.config.model 引用（如 'gpt-4o','claude-opus'）
provider_key     varchar  NOT NULL, FK→provider(key)   服务它的那个供应商
model_identifier varchar  NOT NULL        供应商侧真实模型串（'claude-opus-4-2026...'）
display_name     varchar  NOT NULL
usage_type       varchar  NOT NULL        用量/计费判别器（'anthropic_messages'…）→ 关联 usage_event
default_params   jsonb    NOT NULL default '{}'   可选默认采样参数（缺则用供应商默认）
enabled          boolean  NOT NULL default true
created_at       timestamptz NOT NULL default now()
updated_at       timestamptz NOT NULL default now()
-- 售价(积分单价/加价) → 未来 §10.1
```
**解析 + fail-fast**：`agent.model='gpt-4o' → model('gpt-4o') 且 enabled → provider(解密密钥/端点/vendor→适配器) + model_identifier → 调用`。任一缺失（agent 没配 model / model 不存在或禁用 / provider 没配或没密钥）→ **抛错"模型/供应商未配置"**，绝不回退内置。`usage_event.usage_type = model.usage_type`。
> 多供应商/负载均衡：MVP 一个模型挂一个供应商；将来要 LB → 加 `model_deployment` 绑定表 或 前置网关（§10.5）。

### 5.5 对话域：`conversation` / `run` / `item`
...
agent_key       varchar      NOT NULL, default 'chat'    这次哪个 agent 在跑（resolve_agent 的 key）
...
```
索引：`INDEX(conversation_id, created_at)`；`INDEX(user_id, created_at)`。

**⑪ `item` —— run 内有序事件**（只追加，多态）
```
id          bigint       PK
...
tool_key    varchar      NULL                    type='tool_call'/'tool_output' 时：哪个工具（引用 tool.key）
created_at  timestamptz  NOT NULL default now()
...
> 用量不再放 item（原 `token_usage` 删除），统一进 `usage_event`，靠 `item_id` 关联。

### 5.6 `usage_event` —— 精确用量（**记录，不扣费**；统一记「模型 + 工具」）
...
- **本期只记不扣**：每次模型/工具调用后写一条，记原始用量；**不触发任何积分扣减**（无标价，也无法算 cost——成本=用量×定价，定价属 §10.1/§10.5）。
...
- **预留**（未来计费，§10.1）：`provider` 快照、`billing_status`（异步结算接缝）、定价快照、`credits_charged`、`cost`。
...
三层：`tool`（原子能力，代码实现）→ `agent`（配置实例）→ `app`（应用中心入口，未来）。上层包装下层，同一能力可在每层各现身一次；三入口最终汇流到 `run → item` 同一套执行表。本期落地「对话」入口 + `agent`/`tool`；`app`/应用中心见 §10.4。
...
**测试**：单元（密码哈希/会话/邀请码核销/resolve_agent 三级回退/用量记录）；集成（注册→登录→对话→react_loop 调 tavily→item & usage_event 落库→SSE）；契约（provider 适配层 mock 各 vendor）；迁移（Alembic 升降级 + 种子 admin 幂等 + tool seeding upsert 不覆盖策略）。
...
| Lago / Stripe / TigerBeetle | 预付积分钱包、Credit Grants 批次过期、复式账本两阶段（计费参考，§10.1） |
...
### 10.3 工具体系扩展
- 更多内置工具、外部工具走 OpenAPI/MCP + 通用适配器；工具计费规则上 `tool`（§10.1）。

### 10.4 应用中心
- 前期写死页面；后期模板化+可发布（模板应用/代码应用），`app` 表（ui_kind / backing tool|agent）。

### 10.5 模型层演进（多供应商 / 负载均衡）
...
## 附 术语速查
- **run / item**：一次执行 / 其内有序事件。**opaque token / 会话**：随机串当钥匙 + 服务端存会话行（有状态、可即时吊销）。**代理键**：无语义自增主键；**key 主键**：代码引用的注册表用不可变 key 当 PK。**runtime_pattern**：代码里的编排逻辑（react_loop…）。**usage_type**：用量/计费判别器，决定 usage payload 形状。
